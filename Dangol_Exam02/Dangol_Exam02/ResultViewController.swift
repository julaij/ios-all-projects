...
class ResultViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        enteredLoanAmtOL.text! += String(enteredAmountLabel)
        enteredInterestRateOL.text! += String(interestRateInput) + "%"
        calculatedMonthlyEMI.text! += String(format: "%.2f", calculatedEMI)
        
        imageViewOL.image += UIImage(named: image)


                loanTypeImageView.image = CGAffineTransform(scaleX: 0.5, y: 0.5)
